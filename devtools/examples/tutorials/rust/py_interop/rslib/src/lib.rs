...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn py_greet_rs(name: *const c_char) -> *const c_char {
    let name = unsafe { CStr::from_ptr(name) };
    let name = name.to_str().expect("The message is always valid UTF8");

    let mut res_data = Vec::<u8>::default();
    write!(&mut res_data, "Hello dear {} from rust!", name)
        .expect("Failed to write greeting message");
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn py_free_rs_string(ptr: *const c_char) {
